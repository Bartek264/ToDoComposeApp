...
@ExperimentalMaterialApi
@Composable
fun HandleListContent(tasks: List<ToDoEntity>, navigateToTaskScreen: (taskId: Long) -> Unit) {
	if (tasks.isEmpty()) {
		EmptyComponent()
	} else {
		DisplayItems(toDoList = tasks, navigateToTaskScreen = navigateToTaskScreen)
	}
}
